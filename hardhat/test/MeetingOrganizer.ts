...
describe('MeetingOrganizer Contract Deployment', function () {
    ...
    this.beforeEach(async () => {
        ...
        hardhatContract = await Contract.deploy(_addr1.address);
        owner = _owner;
        addr1 = _addr1;
        addr2 = _addr2;
    })
    ...
    describe('########## ---> Collected Fee and Withdraw Function Testing <--- ##########', function () {
        it('Collected fee amount should equal to zero (owner query)', async function () {
            expect(await hardhatContract.connect(owner.address).queryCollectedFee()).to.equal(0)
        })
        it('Collected fee amount should equal to zero (shareholder query)', async function () {
            expect(await hardhatContract.connect(addr1.address).queryCollectedFee()).to.equal(0)
        })
        it('Collected fee amount query should revert', async function () {
            await expect(hardhatContract.connect(addr2.address).queryCollectedFee()).to.be.revertedWithCustomError(hardhatContract, "CustomERROR_Not_A_ShareHolder");
        })
        it('Cannot withdraw since the collected amount is not greater than zero', async function () {
            await expect(hardhatContract.withdraw()).to.be.revertedWithCustomError(hardhatContract, 'CustomERROR_No_Collected_Fee')
        })
        it('Cannot withdraw since caller is not a shareholder', async function () {
            const otherSignerAddress = await ethers.getSigner(addr2.address);
            expect(hardhatContract.connect(otherSignerAddress)).to.be.revertedWithCustomError(hardhatContract, 'CustomERROR_Not_A_ShareHolder')
        })
